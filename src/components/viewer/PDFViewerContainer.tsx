...
interface PDFViewerContainerProps {
    ...
    onFieldLocked?: (fieldName: string, lockedBy: 'contractor' | 'client') => void; // Callback when field is locked
}
...
const PDFViewerContainer = forwardRef<PDFViewerHandle, PDFViewerContainerProps>(
    ({ documentUrl, xfdfString, readOnly = false, commentsOnly = false, clientSigningMode = false, templateFormFields, toolbarMode = 'annotate', formFields, currentUserRole, onDocumentLoaded, onError, onFieldChange, onFieldLocked }, ref) => {
        ...
        useImperativeHandle(ref, () => ({
            ...
            exportAnnotations: async (fieldValues?: Record<string, string>) => {
                ...
                try {
                    ...
                    // ALWAYS read current values from the PDF form FIRST
                    // Then merge any explicitly passed fieldValues ON TOP
                    // This ensures values from contract creation are preserved + new edits are captured
                    console.log('📖 Reading current field values from PDF form...');
                    let valuesToCommit: Record<string, string> = {};

                    // We always want to read existing values, even if fieldValues is provided
                    {

                        // CRITICAL: First, force commit of any pending field edits
                        // When user is actively typing, values are stored in the DOM input element
                        // but not yet committed to the widget annotation. We must force this commit.
                        console.log('🔄 Step 1: Force committing pending field edits...');

                        // Get the document viewer and attempt multiple commit strategies
                        try {
                            const docViewer = Core.documentViewer;
                            const annotManager = Core.annotationManager;

                            // Strategy 1: Get the focused widget and save its input value BEFORE deselecting
                            const selectedAnnots = annotManager.getSelectedAnnotations();
                            console.log(`   📋 Currently selected annotations: ${selectedAnnots.length}`);

                            for (const annot of selectedAnnots) {
                                if (annot instanceof Core.Annotations.TextWidgetAnnotation ||
                                    annot.getFieldName) {
                                    const fieldName = annot.getFieldName?.() || annot.fieldName;
                                    console.log(`   🎯 Active widget: ${fieldName}`);

                                    // Try to get the input element that PDFTron creates for editing
                                    // PDFTron creates an input/textarea overlay when editing a field
                                    const widgetEl = (annot as any).getCustomData?.('htmlElement') ||
                                        (annot as any).htmlElement;
                                    if (widgetEl && widgetEl.value) {
                                        console.log(`   💡 Found active input value: "${widgetEl.value}"`);
                                        valuesToCommit![fieldName] = widgetEl.value;
                                    }

                                    // Try setValue on the widget before deselecting
                                    const currentValue = annot.getValue?.();
                                    console.log(`   📝 Widget getValue before commit: "${currentValue}"`);
                                }
                            }

                            // Strategy 2: Find PDFTron's text input overlay by looking for specific elements
                            const pdfViewerEl = docViewer?.getScrollViewElement?.()?.closest('.webviewer');
                            if (pdfViewerEl) {
                                // Look for PDFTron's form field input overlay
                                const formInputs = pdfViewerEl.querySelectorAll(
                                    'input.TextInput, textarea.TextInput, [class*="text-input"], [class*="form-field"]'
                                );
                                console.log(`   🔍 Found ${formInputs.length} potential PDFTron form inputs`);

                                formInputs.forEach((input: any, idx: number) => {
                                    if (input.value) {
                                        // Store with a temporary key - will match later
                                        valuesToCommit![`_activeInput_${idx}`] = input.value;
                                    }
                                });
                            }

                            // Strategy 3: Access WebViewer iframe content
                            if (viewerInstance.current) {
                                const iframeDoc = viewerInstance.current.getViewerElement?.()?.contentDocument;
                                if (iframeDoc) {
                                    const iframeInputs = iframeDoc.querySelectorAll('input, textarea');
                                    console.log(`   🔍 Found ${iframeInputs.length} inputs in WebViewer iframe`);
                                    iframeInputs.forEach((input: any, idx: number) => {
                                        if (input.value && input.value.trim()) {
                                            console.log(`   📝 Iframe input[${idx}] value: "${input.value}"`);
                                            valuesToCommit![`_iframeInput_${idx}`] = input.value;
                                        }
                                    });
                                }
                            }

                            // Now deselect to trigger commit
                            annotManager.deselectAllAnnotations();

                            // Also blur document to trigger any remaining commits
                            if (document.activeElement instanceof HTMLElement) {
                                document.activeElement.blur();
                            }

                            // Trigger field manager update if available
                            const fieldManager = annotManager.getFieldManager();
                            if (fieldManager.forceUpdate) {
                                fieldManager.forceUpdate();
                            }
                        } catch (e) {
                            console.warn('   ⚠️ Error during force commit:', e);
                        }

                        // Wait longer for commit to complete
                        await new Promise(resolve => setTimeout(resolve, 500));

                        // STEP 2: Read values from field manager
                        console.log('📋 Step 2: Reading from field manager...');
                        const fields = fieldManager.getFields();
                        for (const fieldName in fields) {
                            const field = fields[fieldName];
                            try {
                                // Skip signature fields - they're handled via annotations
                                if (field.type === 'Sig') {
                                    console.log(`  ⏭️ Skipping signature field: ${fieldName}`);
                                    continue;
                                }

                                const currentValue = field.getValue?.();
                                console.log(`  🔍 Field "${fieldName}" value from manager: "${currentValue}" (type: ${typeof currentValue})`);

                                if (currentValue !== null && currentValue !== undefined && currentValue !== '') {
                                    valuesToCommit[fieldName] = typeof currentValue === 'object'
                                        ? JSON.stringify(currentValue)
                                        : String(currentValue);
                                    console.log(`  ✅ Captured field: ${fieldName} = "${valuesToCommit[fieldName]}"`);
                                }
                            } catch (e) {
                                console.warn(`  ⚠️ Error reading field ${fieldName}:`, e);
                            }
                        }
                        console.log('📊 Fields from field manager:', Object.keys(valuesToCommit).length);

                        // STEP 2.5: Read values directly from DOM input elements (WORKAROUND)
                        // PDFTron creates actual HTML input elements for form fields
                        // These are rendered INSIDE an iframe, so we need to access the iframe's document
                        console.log('📋 Step 2.5: Reading values directly from DOM inputs...');
                        try {
                            // PDFTron renders content inside iframes
                            // We need to find all iframes and search for inputs inside them
                            ...
                            console.log('...........here............');
                            const mainInputs = document.querySelectorAll('input[type="text"], textarea');
                            allInputs.push(...Array.from(mainInputs));
                            console.log(`   📄 Found ${mainInputs.length} inputs in main document`);
                            ...
                            iframes.forEach((iframe, iframeIdx) => {
                                try {
                                    ...
                                    if (iframeDoc) {
                                        const iframeInputs = iframeDoc.querySelectorAll('input[type="text"], input:not([type]), textarea');
                                        console.log(`   📄 Iframe[${iframeIdx}]: Found ${iframeInputs.length} inputs`);
                                        allInputs.push(...Array.from(iframeInputs));
                                    }
                                } catch (e) {
                                    console.log(`   ⚠️ Cannot access iframe[${iframeIdx}] (cross-origin)`);
                                }
                            });

                            console.log(`   🔍 Total DOM inputs found: ${allInputs.length}`);

                            allInputs.forEach((input: any, idx: number) => {
                                const value = input.value;
                                // Try to get the field name from various attributes
                                ...
                                console.log(`   📝 Input[${idx}]: id="${input.id || 'none'}", name="${fieldName}", value="${value}"`);

                                if (value && value.trim() && valuesToCommit) {
                                    // Try to match to an existing field name pattern
                                    const matchingKey = Object.keys(valuesToCommit || {}).find(k =>
                                        k.toLowerCase().includes(fieldName.toLowerCase()) ||
                                        fieldName.toLowerCase().includes(k.toLowerCase()) ||
                                        // Also try matching by field number patterns
                                        fieldName.includes('1') && k.includes('1') ||
                                        fieldName.includes('2') && k.includes('2')
                                    );

                                    if (matchingKey) {
                                        if (valuesToCommit![matchingKey] !== value) {
                                            console.log(`   📝 PDFTron input[${idx}] value: "${input.value}"`);

                                            console.log(`   ✅ Updated ${matchingKey} from DOM: "${value}"`);
                                            valuesToCommit![matchingKey] = value;
                                        }
                                    } else {
                                        // Add as new field using the value
                                        valuesToCommit![fieldName] = value;
                                        console.log(`   ✅ Added from DOM: ${fieldName} = "${value}"`);
                                    }
                                }
                            });
                        } catch (e) {
                            console.warn('   ⚠️ Error reading DOM inputs:', e);
                        }

                        // STEP 3: Also check widget annotations directly for any missed values
                        // Sometimes the value hasn't propagated to field manager yet
                        console.log('📋 Step 3: Checking widget annotations for additional values...');
                        ...
                        for (const annot of allAnnotations) {
                            ...
                            const isWidget = annot.constructor.name.includes('Widget') ||
                                typeof annot.getField === 'function';

                            if (isWidget) {
                                try {
                                    const field = annot.getField?.();
                                    const fieldName = field?.name || annot.fieldName || annot.getFieldName?.();

                                    if (!fieldName) continue;
                                    if (field?.type === 'Sig') continue; // Skip signature fields

                                    // Get value from widget annotation
                                    let widgetValue = annot.getValue?.() || annot.getFieldValue?.();

                                    // Also try getting from the field directly
                                    if (!widgetValue && field) {
                                        widgetValue = field.getValue?.();
                                    }

                                    console.log(`  🔍 Widget "${fieldName}" value: "${widgetValue}"`);

                                    // CRITICAL: ALWAYS prefer widget values over field manager values
                                    // Widget annotations contain the user's LATEST edits
                                    // Field manager may have OLD values from imported XFDF
                                    if (widgetValue) {
                                        const oldValue = valuesToCommit[fieldName];
                                        valuesToCommit[fieldName] = typeof widgetValue === 'object'
                                            ? JSON.stringify(widgetValue)
                                            : String(widgetValue);

                                        if (oldValue && oldValue !== valuesToCommit[fieldName]) {
                                            console.log(`  🔄 Updated from widget: ${fieldName} = "${valuesToCommit[fieldName]}" (was: "${oldValue}")`);
                                        } else if (!oldValue) {
                                            console.log(`  ✅ Captured from widget: ${fieldName} = "${valuesToCommit[fieldName]}"`);
                                        }
                                    }
                                } catch (e) {
                                    console.warn(`  ⚠️ Error reading widget:`, e);
                                }
                            }
                        }

                        console.log('📊 Total fields captured from PDF:', Object.keys(valuesToCommit).length);
                        console.log('📝 All captured values:', valuesToCommit);
                    }

                    // CRITICAL: Merge passed fieldValues ON TOP of read values
                    // This ensures:
                    // 1. Values from contract creation (in XFDF) are preserved
                    // 2. New edits during signing (tracked by DocumentViewerDialog) take precedence
                    if (fieldValues && Object.keys(fieldValues).length > 0) {
                        console.log('🔀 Merging passed fieldValues with PDF values...');
                        console.log('   📝 Passed values:', fieldValues);
                        for (const fieldName in fieldValues) {
                            const oldValue = valuesToCommit[fieldName];
                            const newValue = fieldValues[fieldName];
                            if (oldValue !== newValue) {
                                console.log(`   ✏️ Overwriting ${fieldName}: "${oldValue}" → "${newValue}"`);
                            }
                            valuesToCommit[fieldName] = newValue;
                        }
                        console.log('   ✅ Merged values:', valuesToCommit);
                    }

                    if (valuesToCommit && Object.keys(valuesToCommit).length > 0) {
                        console.log('🔄 Committing field values before export...');

                        // Set each field value using fieldManager.getField().setValue()
                        // This is the CORRECT way per Apryse documentation
                        for (const fieldName in valuesToCommit) {
                            const value = valuesToCommit[fieldName];
                            try {
                                ...
                                if (field) {
                                    ...
                                    console.log(`  ✓ Committing ${fieldName} = "${value}"`);
                        ...
                        console.log('✅ Field values committed successfully');
                    } else {
                        console.log('⚠️ No field values found to commit');
                    }
                    ...
                    */
                    ...
                    // ========================================
                    // CRITICAL WORKAROUND: Inject field values directly into XFDF
                    // ========================================
                    // PDFTron sometimes doesn't include field values in XFDF export
                    // We'll build the <fields> section ourselves using captured values

                    console.log('🔧 Injecting captured field values into XFDF...');

                    // Build the fields section
                    let fieldsSection = '<fields>';
                    let injectedCount = 0;

                    // Use valuesToCommit which we captured earlier (from both field manager and widgets)
                    if (valuesToCommit && Object.keys(valuesToCommit).length > 0) {
                        for (const fieldName in valuesToCommit) {
                            const value = valuesToCommit[fieldName];
                            if (value) {
                                ...
                                fieldsSection += `<field name="${fieldName}"><value>${escapedValue}</value></field>`;
                                injectedCount++;
                                console.log(`  💉 Injected: ${fieldName} = "${value}"`);
                            }
                        }
                    }
                    fieldsSection += '</fields>';

                    console.log(`📊 Total fields injected: ${injectedCount}`);

                    // Now modify the XFDF string
                    let finalXfdfString = xfdfString;

                    if (injectedCount > 0) {
                        // Check if XFDF already has a <fields> section
                        if (xfdfString.includes('<fields>')) {
                            // Replace existing fields section
                            console.log('🔄 Replacing existing <fields> section...');
                            finalXfdfString = xfdfString.replace(/<fields>[\s\S]*?<\/fields>/, fieldsSection);
                        } else {
                            // Insert fields section before </xfdf>
                            console.log('➕ Adding new <fields> section to XFDF...');
                            finalXfdfString = xfdfString.replace('</xfdf>', fieldsSection + '</xfdf>');
                        }

                        console.log('✅ Field values injected into XFDF');
                        console.log('📋 New XFDF length:', finalXfdfString.length);

                        // Verify injection
                        const hasFieldsNow = finalXfdfString.includes('<fields>') && finalXfdfString.includes('<value>');
                        console.log('✅ XFDF now contains <fields> section:', hasFieldsNow);
                    } else {
                        console.log('⚠️ No field values to inject');
                    }

                    // Verify field values are in XFDF
                    const hasFieldValues = finalXfdfString.includes('<fields>') && finalXfdfString.includes('<value>');
                    if (hasFieldValues) {
                        console.log('✅ Final XFDF contains field values!');

                        // Show what values are in the fields section
                        const fieldMatches = finalXfdfString.match(/<field name="([^"]+)"><value>([^<]*)<\/value><\/field>/g);
                        if (fieldMatches) {
                            console.log('📝 Fields in XFDF:', fieldMatches);
                        }
                    } else {
                        console.warn('⚠️ Final XFDF might not contain field values');
                    }
                ...
                try {
                    ...
                    if (widgetAnnotations.length === 0) {
                        ...
                        for (const fieldName in allFields) {
                            ...
                            try {
                                ...
                                // CRITICAL: Read actual ReadOnly flag from field (not hardcoded false!)
                                ...
                                // Also check for lockedBy custom data (field-level locking feature)
                                const firstWidget = field.widgets?.[0];
                                const lockedBy = firstWidget?.getCustomData?.('lockedBy') || null;
                                ...
                                console.log(`  ✓ Extracted from FieldManager: ${fieldName} (${fieldType}) [required: ${isRequired}, readOnly: ${isReadOnly}${lockedBy ? `, lockedBy: ${lockedBy}` : ''}]`);
                            } catch (err) {
                    ...
                    for (const widget of widgetAnnotations) {
                        try {
                            ...
                            const fieldFlags = field?.flags;
                            const isRequired = fieldFlags?.get?.('Required') || false;
                            // CRITICAL: Read actual ReadOnly flag - also check widget.ReadOnly for fallback
                            const isReadOnly = fieldFlags?.get?.('ReadOnly') || (widget as any).ReadOnly || false;
                            const isMultiline = fieldFlags?.get?.('Multiline') || false;
                            const doNotScroll = fieldFlags?.get?.('DoNotScroll') || false;
                            const doNotSpellCheck = fieldFlags?.get?.('DoNotSpellCheck') || false;

                            // Check for lockedBy custom data (field-level locking feature)
                            const lockedBy = (widget as any).getCustomData?.('lockedBy') || null;
                            ...
                            formFields.push(formField);
                            console.log(`  ✓ Extracted: ${fieldName} (${fieldType}) [required: ${isRequired}, readOnly: ${isReadOnly}${lockedBy ? `, lockedBy: ${lockedBy}` : ''}]`);
                        } catch (err) {
            ...
            const initializeViewer = async () => {
                ...
                try {
                    ...
                    if (readOnly && !commentsOnly) {
                        ...
                        // ========================================
                        // LOCK FIELD BUTTON (for field-level locking)
                        // ========================================
                    ...
                    Core.documentViewer.addEventListener('documentLoaded', async () => {
                        ...
                        // ========================================
                        // CLIENT SIGNING MODE: All fields are editable
                        // ========================================
                        // Clients can edit ALL fields when receiving a contract for signature
                        // This allows them to modify any values before signing
                        // Note: Reviewers and approvers still have view-only access via readOnly mode
                        if (clientSigningMode) {
                            console.log('✏️ Client signing mode enabled - ALL fields are editable for client');
                            console.log('🔓 Client can edit all form fields and sign the document');
                        }

                        // ========================================
                        // RESTORE FORMFIELDS FLAGS (ReadOnly, etc.)
                        // ========================================
                        // Apply saved ReadOnly and other flags from formFields prop
                        // This restores flags that were set during contract creation
                        if (formFields && formFields.length > 0) {
                            console.log('📋 [FormFields] Restoring flags from saved formFields...');
                            console.log(`📋 [FormFields] Total formFields: ${formFields.length}`);

                            try {
                                const annotManager = Core.annotationManager;
                                const fieldManager = annotManager.getFieldManager();
                                let readOnlyRestored = 0;
                                let lockedByRestored = 0;

                                for (const fieldDef of formFields) {
                                    if (!fieldDef.name) continue;

                                    // Check if this field has readOnly flag
                                    if (fieldDef.readOnly) {
                                        console.log(`  🔒 [FormFields] Restoring ReadOnly for: ${fieldDef.name}`);

                                        // Find the field and set ReadOnly flag
                                        const field = fieldManager.getField(fieldDef.name);
                                        if (field && field.flags?.set) {
                                            field.flags.set('ReadOnly', true);
                                            console.log(`    ✅ Applied ReadOnly via field.flags`);
                                            readOnlyRestored++;
                                        }

                                        // Also set on widget annotations
                                        const annotations = annotManager.getAnnotationsList();
                                        for (const annot of annotations) {
                                            const annotFieldName = (annot as any).getField?.()?.name ||
                                                (annot as any).fieldName;
                                            if (annotFieldName === fieldDef.name) {
                                                (annot as any).ReadOnly = true;
                                                if ((annot as any).setReadOnly) {
                                                    (annot as any).setReadOnly(true);
                                                }
                                                console.log(`    ✅ Applied ReadOnly to widget annotation`);
                                            }
                                        }
                                    }

                                    // Check for lockedBy custom data (field-level locking)
                                    if (fieldDef.lockedBy) {
                                        console.log(`  🔐 [FormFields] Restoring lockedBy for: ${fieldDef.name} (${fieldDef.lockedBy})`);
                                        ...
                                        for (const annot of annotations) {
                                            const annotFieldName = (annot as any).getField?.()?.name ||
                                                (annot as any).fieldName;
                                            if (annotFieldName === fieldDef.name) {
                                                (annot as any).setCustomData?.('lockedBy', fieldDef.lockedBy);
                                                lockedByRestored++;
                                                console.log(`    ✅ Restored lockedBy custom data`);
                                            }
                                        }
                                    }
                                }

                                console.log('📋 [FormFields] Restoration complete:');
                                console.log(`  📊 ReadOnly flags restored: ${readOnlyRestored}`);
                                console.log(`  📊 LockedBy flags restored: ${lockedByRestored}`);

                                // Redraw annotations to reflect changes
                                if (readOnlyRestored > 0) {
                                    annotManager.drawAnnotationsFromList(annotManager.getAnnotationsList());
                                }

                            } catch (err) {
                                console.error('❌ [FormFields] Error restoring flags:', err);
                            }
                        }

                        // ========================================
                        // FIELD-LEVEL LOCK ENFORCEMENT
                        // ========================================
                        // Check each widget's lockedBy custom data and enforce read-only
                        // for fields locked by the OTHER party (contractor vs client)
                        // This allows selective field locking between parties
                        if (currentUserRole && !readOnly) {
                            console.log('🔒 [FieldLock] Starting field-level lock enforcement...');
                            console.log(`🔒 [FieldLock] Current user role: ${currentUserRole}`);

                            try {
                                const annotManager = Core.annotationManager;
                                const annotations = annotManager.getAnnotationsList();

                                let lockedForUserCount = 0;
                                let lockedByUserCount = 0;
                                let unlockedCount = 0;
                                ...
                                for (const annot of annotations) {
                                    ...
                                    const isWidget = annot instanceof Core.Annotations.WidgetAnnotation ||
                                        annot.constructor.name.includes('Widget') ||
                                        typeof (annot as any).getField === 'function';

                                    if (!isWidget) continue;

                                    const field = (annot as any).getField?.();
                                    const fieldName = field?.name || (annot as any).fieldName || (annot as any).getFieldName?.();

                                    if (!fieldName) continue;

                                    // Get the lockedBy custom data
                                    const lockedBy = (annot as any).getCustomData?.('lockedBy');

                                    if (!lockedBy) {
                                        console.log(`  🔓 [FieldLock] ${fieldName}: NOT LOCKED`);
                                        unlockedCount++;
                                        continue;
                                    }
                                    ...
                                    if (lockedBy === currentUserRole) {
                                        ...
                                        console.log(`  🚫 [FieldLock] ${fieldName}: LOCKED BY OTHER PARTY (${lockedBy}) - READ-ONLY ENFORCED`);
                                        if (field?.flags?.set) {
                                            field.flags.set('ReadOnly', true);
                                        }
                                        annot.ReadOnly = true;
                        ...
                        // ========================================
                        // FIELD CHANGE EVENT LISTENER (CRITICAL FOR VALUE TRACKING)
                        // ========================================
                        // This MUST be set up for ALL scenarios (signing, viewing, etc.)
                        // Previously this was only inside the formFields creation block,
                        // which caused field changes during signing to not be tracked!
                        if (onFieldChange) {
                            console.log('📡 Setting up field change listener for value tracking...');
                            ...
                            // Also listen for value changes on widget annotations 
                            // This catches changes that might not trigger fieldChanged
                            Core.annotationManager.addEventListener('annotationChanged', (annotations: any[], action: string) => {
                                if (action === 'modify') {
                                    annotations.forEach((annot: any) => {
                                        // Check if this is a text widget with a new value
                                        if (annot.getValue && annot.getFieldName) {
                                            const fieldName = annot.getFieldName();
                                            const value = annot.getValue();
                                            if (fieldName && value !== undefined) {
                                                console.log('📝 Widget value changed:', fieldName, '→', value);
                            ...
                            console.log('✅ Field change listeners enabled');
                        }
